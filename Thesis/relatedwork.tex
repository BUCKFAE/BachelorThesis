...
~\autocite{Lee_Togelius_2017}, a node represents the worst case scenario that would occur as a result of the current choice. 
...
\label{eq:minmax-eval-func}
    ...
    0.3 \cdot \text{depth}
\end{equation}
A YouTube-Video released by \emph{Rempton Games} ~\autocite{RemptonGames:PokemonAI} alters this evaluation function
...
The first agent described by ~\autocite{Lee_Togelius_2017} won 70 out of 90 total games against a random agent,
...
at ~\autocite{Github:pmariglia-showdown}. This approach uses the \textit{Expectiminimax}-Algorithm and takes
...
The YouTube-Video released by \emph{Rempton Games} ~\autocite{RemptonGames:PokemonAI} also introduces two Rule based agents.
...
The implementation for this agent can be found at ~\autocite{PokeEnv:Baselines}. This agent uses a few simple rules
...
\begin{table}[h]
    \centering
        \begin{tabular}{|l|l|l|l|l|l|}
            ...
            \hline 
        \end{tabular}
        \caption{Denotes how many games the Agent in the column won against the agent in the row ~\autocite{RemptonGames:PokemonAI}}
...
agent assumes its opponent is a \emph{One Turn Lookahead}-agent and simulates accordingly ~\autocite{Lee_Togelius_2017}.
...
One year after the publication of ~\autocite{Lee_Togelius_2017}, the authors of ~\autocite{GottaTrainEmAll} 
improved this design. They used \ac{PPO} ~\autocite{schulman2017proximal} to train an agent. They also
used embeddings to better represent a Pokémon. Data available at ~\autocite{Kaggle:NewYorkData} was used
...
~\autocite{grover2016node2vec} which creates embeddings from graph data in similar to
...
\begin{table}[h]
    \centering
        \begin{tabular}{|l|p{0.7\textwidth}|}
            ...
            \hline
        \end{tabular}
        \caption{Similar Pokémon within the embedding space of ~\autocite{GottaTrainEmAll}}
...
The authors of ~\autocite{GottaTrainEmAll} trained the agent against a random agent, a default agent 
...
\begin{table}[h]
    \centering
        \begin{tabular}{|l|l|}
            ...
            \hline
        \end{tabular}
        \caption{Average epoch rewards after training convergence for opponent agents ~\autocite{GottaTrainEmAll}}
        \label{tbl:Gotta-Performance}
\end{table}
The authors of ~\autocite{GottaTrainEmAll} describe their final agent as flawed as while the agent learned 
...
In 2019, the authors of ~\autocite{Huang_Lee_2019} published a paper titled \emph{A Self-Play Policy 
...
Similar to
OpenAI's Dota AI ~\autocite{OpenAI_dota}, the agent is represented using an actor-critic neural network.
...
the RL-Agent described in ~\autocite{GottaTrainEmAll}. Futhermore, the authors played 1.000 games against
...
\begin{table}[h]
    \centering
        \begin{tabular}{|l|l|l|}
            ...
            \hline
        \end{tabular}
        \caption{Performance of the agent developed by ~\autocite{Huang_Lee_2019}}
