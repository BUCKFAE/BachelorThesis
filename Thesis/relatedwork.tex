...
\todo{Explosion: Good decision but Pokémon faints}
...
\section{Rulebased agents}
\label{sec:related-rulebased}

\section{LastVideo}:
\begin{itemize}
    \item Used Random/MaxDamage
    \item Rulebased: (Called: Smart Damage Player) type / Speed for disadvantage. If true -> Switch to Pokemon best matchup
     Simplifed damage calculation: Use Move most Damage
    \item MiniMax: Score increases when damage / knockout opponent, decreases: Damage. Flaw: Hazards, Status
    One: As good as Rulebased. More: Performed worse. Possible explainations: Bugs, gametree does not apply as well
    to pokemon as not alternate rather simultanious, not enough information (hazard, stats)
    \item Author states that Pokemon is bad for AI
    \item SimpleHeuristicsPlayer: Rulebased but more sophisticated.
    \item 1.000 Battles
\end{itemize}
