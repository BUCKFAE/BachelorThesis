...
were generated, and the team information was stored. Next, the board rating was calculated.
\begin{equation}
  \text{member-score (Pokémon: e)} = \sum_{j \in \text{Enemy Pokémon countered by e}} 1
\end{equation}
\begin{equation} 
  \text{team-score (Team: p)} = \sum_{e \in p} \text{member-score}(e)
\end{equation}
\begin{equation}
  \text{board rating (Team: p1, Team: p2)} = \text{team-score(p1)} - \text{team-score(p2)}
\end{equation}
...
of the matchups being almost fair according to our metric. Secondly, the graphic 
...
\todo{Dynamaxing will likely be removed next generation?}
...
\todo{If Memezboi69 is introduced in Dynamax, no introduction needed here!}
