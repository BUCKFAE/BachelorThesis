...
remaining Pokémon of the agent.  

\begin{itemize}
  \item First Game: Shuckle vs Mr.Mime: Spammed Hazards, Mr. Mime combination of boosting and rapid spin
  \item Second Game: Gengar boosted to stage two managed to one shot four remaining pokemon. Sticky Web 
  was set. No Pokemon was able to outspeed (aside from Klefki)
  Should have sent in Klefki first, Poison Gengar. Probably not winnable anyway
  \item Third game: Bot: Boosting Obstagon (yay). Probably one stage to far, bur damage. Close game, 
  Hazards 
\end{itemize}