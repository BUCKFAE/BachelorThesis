...
class MinMaxNode:
    ...
    def build_tree_below_node(self):
        ...
        fainted_after_turns_p1 = current_matchup.expected_turns_until_faint(self.own_species)
        fainted_after_turns_p2 = current_matchup.expected_turns_until_faint(self.enemy_species)

        # # logger.info(f'Turns until {self.own_species} faints: {fainted_after_turns_p1}')
        # # logger.info(f'Turns until {self.enemy_species} faints: {fainted_after_turns_p2}')

        # TODO: If both pokemon faint after the same amount of attacks, take speed into account!
        if fainted_after_turns_p1 <= fainted_after_turns_p2:
            # logger.info(f'{self.own_species} faints before {self.enemy_species}')
            total_hp_p2 = current_matchup.pokemon_2.current_hp
        ...
        if fainted_after_turns_p2 < fainted_after_turns_p1:
            ...
            total_hp_p1 = current_matchup.pokemon_1.current_hp
